...
if (isset($_SESSION['user']) && $info['regstatus'] == 1) { 
    ...
    $getposts = $con->prepare("SELECT * FROM lessons where cat_id = ? AND youtube = 1 AND orders <= ? ORDER BY lesson_id  DESC");
        ...
        <div class="row mb-5">
          <?php foreach($events as $event){ ?>
            <div class="col-lg-6 col-md-6">
              <div class="member">
                <div class="pic"><img src="assets/img/liv3.jpg" class="img-fluid" alt=""></div>
                <div class="member-info">
                <?php 
                                    $date    = new DateTime($event['lesson_data']);
                                ...
                                ?>
                    <h4><?php echo $event['lesson_name'];?> </h4>
                    <span> <?php echo $result2. '/' . $result . '/' . $result3  ;?></span>
                    <p><?php echo $event['lesson_description'];?></p>
                    <a href="youtube.php?lesson_id=<?php echo $event['lesson_id']; ?>" class="text-left">الذهاب للبث</a>
                    </div>
                </div>
            </div>
            <?php } ?>
